...
const Second = ({
    ...
    const onSendCode = (type) => {
        const category = type === "phone" ? "phone": "email";
        setLoading(type)
        dispatch(sendVerificationCode(category))
            .then(res => {
        ...
        <div className={`nth-box pass-box step${step}`}>
            ...
            <div>
                ...
                <div className='sms-options'>
                    <div className={loading? 'loading' : ''} onClick={() => onSendCode('phone')}>
                        <div className='flex flexCenter alignCenter gap10 wrapBlock' style={{minWidth: "250px"}}>
                            <div className='icon'>
                                <ProfilePhoneIcon />
                            </div>
                        ...
                        {loading === 'phone' && <div className="flexCenter alignCenter btn-load"><CircularProgress/></div>}
                    </div>
                    <div className={loading? 'loading' : ''} onClick={() => onSendCode('email')}>
                        <div className='flex flexCenter alignCenter gap10 wrapBlock' style={{minWidth: "250px"}}>
                            <div className='icon'>
                                <EmailSvgIcon/>
                            </div>
                        ...
                        {loading === 'email'  && <div className="flexCenter alignCenter btn-load"><CircularProgress/></div> }
                    </div>
                </div>
            </div>
        </div>

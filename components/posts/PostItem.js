...
export default function PostItem({data, compact, all}) {
    ...
    return (
        <div className={`nth-blog ${compact ? "compact" : ""}`}>
            <div className='img'>
                <Image src={data.image} width={368} height={250} blurDataURL={data.image} loader={ImageLoader} loading="lazy" alt='Blog'/>
            </div>
            <div className='blog-content'>
                <div className='flexBetween font20 weight500 top-part mb20'>
                    <span className='secondary'>{data.category_id === 1 ? "Blogs" : "News"}</span>
                    {!all && <Link href="/category/blogs" className='primary60 font20 weight500 underline'>View All</Link>}
                </div>
                ...
                {!compact && <div className="description primary60 mb20 font16"
                                  dangerouslySetInnerHTML={{__html: data.body}}
                >
                </div>}
                <div>
                    <Link
                        href={`/post?slug=${data.slug}&category_id=${data.category_id}`}

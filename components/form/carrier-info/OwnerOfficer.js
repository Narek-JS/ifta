...
export default function OwnerOfficer({
    ...
    const formik = useFormik({
        initialValues: {
            ...initialFormValues,
            ...extraInitialValues,
            ...(!Boolean(baseStateId === 89 || baseStateId === 87) && {ssn: ""}),
        },
        ...
        validationSchema: yup.object({
            ...(!Boolean(baseStateId === 89 || baseStateId === 87) && {ssn: schemas.ssn}),
    ...
    const selectAutoFillAdresses = () => {
        ...
        formik.setValues({
            ...
            state: allStates?.find(item => item?.app?.toUpperCase() === usdotValues.content.carrier.phyState.toUpperCase())
        });
    };
                ...
                {active ? <form className="relative mb20" ref={formRef}>
                    <Fade>
                        <div className="inputsContainer flex wrap alignEnd gap20">
                                ...
                                el.type.name === 'date' ? (
                                    <div className="dateInput m-year-Input" key={el.id}>
                                        <p className="helper mb5 bold500 font16 line24">
                                            {el.label || "[ NO LABEL ]"}
                                            {el.validation[0]?.conditions !== "Not Required" ? <sup className="red font16">*</sup> : ""}
                                        </p>
                                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                                            <DatePicker
                                                ...
                                                maxDate={dayjs(String(new Date(`12-31-${new Date().getUTCFullYear() - 18}`)) )}
                                                renderInput={(params) => (
                                                    <TextField
                            ...
                            {!Boolean(baseStateId === 89 || baseStateId === 87) && 
                                <Fragment>
                                    <InputField
                            ...
                            <InputField
                                className={classNames("inputField", {
                                    'disabled': autoFillHomeAddress && Boolean(usdotValues.content.carrier.phyState)
                                })}
                                error={formik.touched?.state && formik.errors?.state}
                                label="State"
                                select={true}
                                required={true}
                                {...((autoFillHomeAddress === true && Boolean(usdotValues.content.carrier.phyState) === true) && {
                                    value: allStates?.find(item => item?.app?.toUpperCase() === usdotValues.content.carrier.phyState.toUpperCase())?.state,
                                    disabled: Boolean(usdotValues.content.carrier.phyState)
                                })}
                                {...(autoFillHomeAddress === false && {
                                    element: <Autocomplete
                                        onChange={(e, value) => {

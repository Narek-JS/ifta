import { clearVehiclesErrorMessage, deleteVehicle, editVehicle, getDataByVin, getPermitDetails, getVehicles, removeVehicle, selectOtherExtraValues, selectVehicles, selectVehiclesErrorMessage, storeVehicle, updateVehicle } from "@/store/slices/resgister";
...
export default function Vehicles({ width, fuelType, extraFields, permit_id, cost }) {
    ...
    const handleStoreVehicle = (values) => {
        setLoading(true);
    ...
    const handleDelete = (id) => {
        dispatch(setPopUp({
            ...
            popUpAction: () => {
                return dispatch(deleteVehicle({ id, permit_id }))
                    .then(res => {
                        dispatch(getPermitDetails(permit_id));
    ...
    return (
        <div className="ownerOfficer" ref={ownerOfficerRef}>
            ...
            <div className="ownerOfficerMain">
                ...
                {active && (
                    <form className="relative mb20" ref={formRef}>
                        <div className="inputsContainer flex wrap alignEnd gap20">
                            { extraFields?.map((el) => {
                                return el.type.name === 'date' ? (
